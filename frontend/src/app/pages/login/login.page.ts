  ...
  template: `
    <div class="min-h-screen flex items-center justify-center bg-gradient-to-br from-greek-900 via-greek-800 to-greek-600 px-4">
      ...
      <div class="relative bg-white rounded-2xl shadow-2xl p-10 w-full max-w-sm text-center">
        ...
        <!-- Error message -->
        @if (error()) {
          <div class="mb-4 rounded-xl bg-red-50 border border-red-200 px-4 py-3 text-sm text-red-700 text-left">
            {{ error() }}
          </div>
        }
        ...
        <button
          ...
          [disabled]="loading()"
          class="w-full flex items-center justify-center gap-3 bg-white border border-surface-200 hover:border-surface-300 hover:shadow-md active:bg-surface-50 disabled:opacity-50 disabled:cursor-not-allowed text-surface-700 font-medium rounded-xl px-4 py-3 transition-all shadow-sm"
        >
          @if (loading()) {
            <!-- Spinner -->
            <svg class="w-5 h-5 animate-spin text-surface-400" fill="none" viewBox="0 0 24 24">
