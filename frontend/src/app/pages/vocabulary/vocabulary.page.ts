        ...
        <div class="max-w-4xl mx-auto space-y-4">
          @for (bookGroup of groupedVocab(); track bookGroup.book.id) {
            ...
            <div class="flex items-center gap-4 pt-8 pb-2">
              ...
              <h2 class="font-serif text-lg font-bold text-surface-500 uppercase tracking-widest shrink-0">{{ bookGroup.book.title }}</h2>
              <div class="flex-1 h-px bg-surface-300"></div>
            </div>
            ...
            @for (chapterGroup of bookGroup.chapters; track chapterGroup.chapter.id) {
              ...
              <!-- Chapter separator -->
              <div [id]="chapterGroup.chapter.id" class="flex items-center gap-4 pt-6 pb-1 scroll-mt-8">
                <div class="flex-1 h-px bg-surface-100"></div>
                <div class="text-center shrink-0">
                  <span class="text-xs font-bold uppercase tracking-widest text-greek-600 block mb-0.5">Chapter {{ chapterGroup.chapter.order }}</span>
                  <h3 class="font-serif text-xl font-semibold text-greek-900 m-0">{{ chapterGroup.chapter.title }}</h3>
                  <span class="text-xs text-surface-400 font-medium">{{ chapterGroup.words.length }} words</span>
                </div>
                <div class="flex-1 h-px bg-surface-100"></div>
              </div>
    ...
    <ng-template #wordCard let-word let-showSource="showSource">
      ...
      >
        ...
        <div class="min-w-0 flex-1">
          <p class="font-serif text-xl font-semibold text-greek-900 leading-tight mb-0.5 truncate" [title]="word.greek">{{ word.greek }}</p>
