...
# ---------------------------------------------------------------------------
# set-curriculum-selection Cloud Function
# ---------------------------------------------------------------------------

resource "google_cloudfunctions2_function" "set_curriculum_selection" {
  project     = var.project_id
  location    = var.region
  name        = "set-curriculum-selection"
  description = "Updates a user's selected chapter variant for a curriculum slot."

  build_config {
    runtime     = "python311"
    entry_point = "set_curriculum_selection_fn"

    environment_variables = {
      GOOGLE_FUNCTION_SOURCE = "fn_set_curriculum_selection.py"
    }
  ...
  service_config {
    available_memory               = var.set_curriculum_selection_function_memory
    available_cpu                  = var.set_curriculum_selection_function_cpu
    timeout_seconds                = var.set_curriculum_selection_function_timeout
    max_instance_count             = 2
    min_instance_count             = 0
    all_traffic_on_latest_revision = true
...
# Allow API Gateway SA to invoke set-curriculum-selection.
resource "google_cloud_run_service_iam_member" "set_curriculum_selection_invoker" {
  project  = var.project_id
  location = var.region
  service  = google_cloudfunctions2_function.set_curriculum_selection.name
  role     = "roles/run.invoker"
  member   = "serviceAccount:${google_service_account.api_gateway_sa.email}"
}

# ---------------------------------------------------------------------------
# mark-onboarding-seen Cloud Function
# ---------------------------------------------------------------------------

resource "google_cloudfunctions2_function" "mark_onboarding_seen" {
  project     = var.project_id
  location    = var.region
  name        = "mark-onboarding-seen"
  description = "Marks an onboarding item as seen for an active user."

  build_config {
    runtime     = "python311"
    entry_point = "mark_onboarding_seen_fn"

    environment_variables = {
      GOOGLE_FUNCTION_SOURCE = "fn_mark_onboarding_seen.py"
    }
  ...
  service_config {
    available_memory               = var.mark_onboarding_seen_function_memory
    available_cpu                  = var.mark_onboarding_seen_function_cpu
    timeout_seconds                = var.mark_onboarding_seen_function_timeout
    max_instance_count             = 2
    min_instance_count             = 0
    all_traffic_on_latest_revision = true
...
# Allow API Gateway SA to invoke mark-onboarding-seen.
resource "google_cloud_run_service_iam_member" "mark_onboarding_seen_invoker" {
  project  = var.project_id
  location = var.region
  service  = google_cloudfunctions2_function.mark_onboarding_seen.name
  role     = "roles/run.invoker"
  member   = "serviceAccount:${google_service_account.api_gateway_sa.email}"
}
